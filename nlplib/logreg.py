from nlplib import evaluation

import torch
import random
import numpy as np
from scipy.special import logsumexp
import matplotlib.pyplot as plt
from torch import optim

from utils.system import parse_params

def build_linear(X_tr, Y_tr, method):
    '''
    Build a linear model in torch

    :param X_tr: the set of training documents
    :param Y_tr: the set of training labels
    :returns: PyTorch linear model
    :rtype: PyTorch model
    '''

    size1 = X_tr.shape[1]
    size2 = len(set(Y_tr))

    #print(size1, size2)
    args = parse_params()
    method = args.method

    if args.method == 'softmax_linear_model':

        print('Running ' + args.method + '...')
        model = torch.nn.Sequential()
        model.add_module("Linear",torch.nn.Linear(size1, size2, bias = True))

    else:
        print('Running ' + args.method + '...')
        model = torch.nn.Sequential(
            torch.nn.Linear(size1, 100),
            torch.nn.ReLU(),
            torch.nn.Dropout(p=0.2),
            torch.nn.Linear(100, size2)
        )
    
    return model

def log_softmax(scores):
    '''
    Compute the (log of the) softmax given the scores

    Hint: Use logsumexp

    :param scores: output of linear model as a numpy array
    :returns: the softmax result
    :rtype: numpy array
    '''
    softmax_result = scores - logsumexp(scores, axis=1, keepdims=1)
    #print(np.exp(softmax_result))
    return softmax_result

def nll_loss(logP, Y_tr):
    '''
    Compute the neg-log likelihood loss from log softmax probabilities, averaged across documents

    return the loss in a number
    :param logP: output of log softmax
    :param Y_tr: the set of training labels
    :returns: the NLL loss
    :rtype: float
    '''
    neg_log_loss = np.zeros(logP.shape[0])
    for i, v in enumerate(logP):
        neg_log_loss[i] = -v[Y_tr[i]]
    return np.average(neg_log_loss)

"""
def train_model(loss, model, X_tr_var, Y_tr_var,
                num_its = 200,
                X_dv_var = None,
                Y_dv_var = None,
                status_frequency=10,
                optim_args = {'lr':0.002,'momentum':0},
                param_file = 'best.params'):

    model.train()

    # initialize optimizer
    optimizer = optim.Adam(model.parameters(), **optim_args)
    #optimizer = optim.Adam(model.parameters(), **optim_args)

    losses = []
    accuracies = []

    for epoch in range(num_its):
        # set gradient to zero
        optimizer.zero_grad()
        # run model forward to produce loss
        #Y_tr_var = Y_tr_var.long()
        #print(X_dv_var.type(), 'afadsfdsfasdf')
        output = loss.forward(model.forward(X_tr_var),Y_tr_var)
        # backpropagate and train
        output.backward()
        optimizer.step()

        losses.append(output.item())

        # write parameters if this is the best epoch yet
        if X_dv_var is not None:

            model.eval()

            # run forward on dev data
            _, Y_hat = model.forward(X_dv_var).max(dim=1)
            # compute dev accuracy
            acc = evaluation.acc(Y_hat.data.numpy(),Y_dv_var.data.numpy())
            # save
            if len(accuracies) == 0 or acc > max(accuracies):
                state = {'state_dict':model.state_dict(),
                         'epoch':len(accuracies)+1,
                         'accuracy':acc}
                torch.save(state,param_file)
            accuracies.append(acc)

            model.train()

        # print status message if desired
        if status_frequency > 0 and epoch % status_frequency == 0:
            print("Epoch "+str(epoch+1)+": Dev Accuracy: "+str(acc))

    # load parameters of best model
    checkpoint = torch.load(param_file)
    model.load_state_dict(checkpoint['state_dict'])
    
    return model, losses, accuracies
"""

def train_model_batch(loss, model, X_tr, Y_tr,
                num_its = 200,
                X_dv = None,
                Y_dv = None,
                status_frequency=10,
                optim_args = {'lr':0.002,'momentum':0},
                param_file = 'best.params',
                batch_size=128 ):

    model.train()

    # initialize optimizer
    optimizer = optim.Adam(model.parameters(), **optim_args)
    #optimizer = optim.Adam(model.parameters(), **optim_args)

    losses = []
    accuracies = []

    X_dv_var = torch.from_numpy(X_dv).float()
    Y_dv_var = torch.from_numpy(Y_dv)

    for epoch in range(num_its):

        model.train()

        for batch in data_iterator(list(range(X_tr.shape[0])), batch_size):
            x_train = X_tr[batch]
            y_train = Y_tr[batch]
            X_tr_var = torch.from_numpy(x_train).float()
            Y_tr_var = torch.from_numpy(y_train)

            #print(X_tr_var.shape, Y_tr_var.shape)

            # set gradient to zero
            optimizer.zero_grad()
            # run model forward to produce loss
            output = loss.forward(model.forward(X_tr_var),Y_tr_var)
            # backpropagate and train
            output.backward()
            optimizer.step()

            #losses.append(output.item())

        model.eval()

        # run forward on dev data
        _, Y_hat = model.forward(X_dv_var).max(dim=1)
        # compute dev accuracy
        acc = evaluation.acc(Y_hat.data.numpy(),Y_dv_var.data.numpy())
        # save
        if len(accuracies) == 0 or acc > max(accuracies):
            state = { 'state_dict':model.state_dict(),
                      'epoch':len(accuracies)+1,
                      'accuracy':acc}
            torch.save(state,param_file)
        accuracies.append(acc)

        # print status message if desired
        #if status_frequency > 0 and epoch % status_frequency == 0:
        print("Epoch "+str(epoch+1)+": Dev Accuracy: "+str(acc))

    # load parameters of best model
    checkpoint = torch.load(param_file)
    model.load_state_dict(checkpoint['state_dict'])
    
    return model, losses, accuracies

def data_iterator(data, batch_size):

    random.shuffle(data)
    start = 0
    while True:
        end = start + batch_size
        batch = data[start:end]
        yield batch
        if end >= len(data):
            break
        start = end


def plot_results(losses, accuracies):
    
    fig,ax = plt.subplots(1,2,figsize=[12,2])
    ax[0].plot(losses)
    ax[0].set_ylabel('loss')
    ax[0].set_xlabel('iteration');
    ax[1].plot(accuracies);
    ax[1].set_ylabel('dev set accuracy')
    ax[1].set_xlabel('iteration');
